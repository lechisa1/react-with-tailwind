...
const UserRegisteration = ({ existingUser, onClose }) => {
  ...
  const userApi = "http://localhost:4000/users/api/register";
    ...
    try {
      if (existingUser) {
       const response= await axios.put(
          `http://localhost:4000/users/api/${existingUser.id}`,
          formData,{headers: { Authorization: `Bearer ${token}` }}
        );
        setSuccess(response.data.message);
        toast.success(response.data.message);
      } else {
        const response=await axios.post("http://localhost:4000/users/api/register", formData,{headers: { Authorization: `Bearer ${token}` }});
        setSuccess(response.data.message);
        toast.success(response.data.message);
        setFormData({
          first_name: "",
      ...
      toast.error(err.response?.data?.error || "Failed to register user. Try again.");
    } finally {
      setLoading(false);
    }
    onClose();
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-4xl mx-auto">
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex justify-center mt-6">
            ...
            >
              {loading ? "Submitting..." : existingUser ? "Update User" : "Register User"}
            </button>
          </div>
        </form>
      </div>
